...
if __name__ == '__main__':
	...
	for repo in available_repos:
		# first 'any' checks for instance of repo in current images (repo'_' assures no confusion in situations such as *bowtie_version* and *bowtie2_version*)
		# second 'any' checks that if first clause passes, the current image has the most recent version
		most_recent_tag = get_most_recent_tag(repo)
		if most_recent_tag is None:
			continue
		if not ( any(repo+'_' in image for image in current_images) or 
			any(repo+'_'+most_recent_tag == image for image in current_images) ):
